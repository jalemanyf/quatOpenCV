...
class StareDetector:
    ...
    def check_frame(self, frame):
        ...
        if isinstance(retinaface_detected_faces, dict):
            faces = [
                dlib.rectangle(
                    #max(df['facial_area'][0]-10, 0),
                    #max(df['facial_area'][1]-10, 0),
                    #min(df['facial_area'][2]+10, frame.shape[0]),
                    #min(df['facial_area'][3]+10, frame.shape[1])
                    df['facial_area'][0],
                    df['facial_area'][1],
                    df['facial_area'][2],
                    df['facial_area'][3],
        ...
        for face in faces:
            ...
            #print("obj:", obj)
             if obj:  # Check if obj is not empty
                ...
                # If obj is empty, set default values or handle the case as needed
                dominant_emo = "Not detected"
                emotions = {}
                dominant_race = "Not detected"
                age = 0
                gender = "Not detected"
                gender_prob = 0.0
                faces_info.append({
